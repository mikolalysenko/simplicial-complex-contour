...
function createCellPolygonizer(d) {
  ...
  var str = proc.toString().replace(/\n/g,"").replace("anonymous", "cellPolygonizer_" + d)
  allStr += '\n' + str + ','

  return proc(pool)
}

createCellPolygonizer(0)
createCellPolygonizer(1)
createCellPolygonizer(2)
createCellPolygonizer(3)

console.log(allStr + '\n]')

function getPolygonizer(d) {
  return createCellPolygonizer(d)
}